<template>
  <div class="box box-widget">
        ...
        <div v-if="busca != 'false'" class="col-md-4 col-sm-6">
          <form method="GET" class="form-horizontal" :action="url">
            <div class="input-group">
              <input type="text" name="busca" class="ml-2 form-control" placeholder="Buscar por" :value="busca" />

              <span class="input-group-btn">
                <button type="submit" class="btn btn-primary btn-flat h-100">
                  <i class="fa fa-search"></i>
                </button>
                <a :href="url" class="btn btn-default btn-flat h-100" data-toggle="tooltip" title="Limpar Busca">
    ...
    <div class="box-body">
      ...
      <form v-if="items.data.length > 0" :class="formClass" v-on:submit.prevent="confirmDelete"
        :action="url + '/destroy'" method="POST">
          ...
          <thead>
            <tr>
              <th v-if="!notDeletable" align="center" width="50px">
                <input class="checkbox-delete" id="checkbox-delete-origin" type="checkbox" name="registro[]"
                  data-toggle="tooltip" title="Marcar tudo" v-on:click="checkAll" @click="checkDelete" />
              </th>
          ...
          <tbody>
            <tr v-for="(item, index) in items.data" :key="index">
              <th v-if="!notDeletable" align="center" class="v-middle">
                <input @click="checkDelete" class="checkbox-delete" type="checkbox" name="registro[]"
                  :value="item.id" />
              </th>
              ...
              <td v-for="(field, index) in item" :key="index" class="v-middle"
                ...
                <span v-if="index === 'image'">
                  <img :src="urlsistem + '/' + field" style="max-width: 150px; max-height: 150px" />
                </span>
                ...
                <span v-else-if="field.type == 'config'" class="badge" style="font-size: 14px"
                  :class="'badge-' + field.style">{{ field.text }}</span>
                <span class="mr-2 px-2 py-0 rounded-pill badge" v-else-if="isMultiBadge(field)"
                  v-for="(campo, index) in field" :key="index" :class="'badge-' + campo.status">
                  <span>{{ campo.text }}</span>
                </span>
                <span v-else-if="field.type == 'badge'" class="badge" :class="'badge-' + field.status">{{ field.text
                  }}</span>

                <span v-else-if="field.type == 'action'" v-for="(action, index) in item.actions" :key="index">
                  <a data-toggle="tooltip" class="btn btn-flat ml-10" :class="'btn-' + action.color" :href="action.path"
                    :title="action.label">
              ...
              <td class="v-middle" v-if="actions && actions != 'false'">
                <span v-for="(action, index) in actions" :key="index">
                  <a data-toggle="tooltip" class="btn btn-flat ml-10" :class="'btn-' + action.color"
                    :href="url + '/' + actionUrl(action.path, item.id)" :title="action.label">
                    <i :class="action.icon"></i> </a>&nbsp;
                </span>
              </td>
            </tr>
          ...
          <tfoot>
            <tr>
              <td :colspan="titles.length + 2" align="left" vertical-align="center">
                <button v-if="!notDeletable" type="submit" class="mv-22 btn btn-flat btn-danger"
